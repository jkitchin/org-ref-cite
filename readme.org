...
This key does not exist, so the key is red:  [cite/t:@rose-2019-pybliom;]
...
With a single prefix arg, you will be prompted to choose a reference and a style. [cite:@rose-2019-pybliom]
...
Check out the tooltips with prefix/suffix text: [cite/t:Thi s is before @rose-2019-pybliom and after;]
...
#+BEGIN_SRC emacs-lisp
(org-ref-cite :fetcher github :repo "jkitchin/org-ref-cite" :files (:defaults))
#+END_SRC
...
- - Ivy is required for this, as it has built-in contextual actions. While [[https://github.com/oantolin/embark][Embark]] offers the same for completing-read, completing-read does not include actions out-of-box.

- These dependencies are pretty non-negotiable.
...
#+BEGIN_SRC emacs-lisp
(cl-loop for capability in '(activate export follow insert) collect (list capability (org-cite-processor-has-capability-p 'org-ref-cite capability)))
#+END_SRC
